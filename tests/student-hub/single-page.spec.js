...
test.describe('single page tests', async () => {
    ...
    test("apply webform submission", async ({ page }) => {
        ...
        await page.locator("button.button:has-text('Submit')").click()
        await expect(page.locator("//h3[contains(@class, 'Webform__Heading-sc') and text()='Success! Form submitted.']")).toBeVisible()
        await page.locator("a.button:has-text('Ok')").click()
    })
})